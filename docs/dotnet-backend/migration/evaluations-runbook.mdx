...
:::caution Illustrative SP names
The stored procedure names below follow the naming convention observed in the codebase but are **representative examples, not verified names**. Run `npm run parse:dotnet:sprocs` against the actual source to extract the real SP inventory, then cross-reference in the [Stored Procedures](/docs/dotnet-backend/data-access/stored-procedures) page.
:::

| SP Name (illustrative) | Called By | PostgreSQL Strategy |
|------------------------|-----------|---------------------|
