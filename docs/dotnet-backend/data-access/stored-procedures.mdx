...
The MyEvaluations SQL Server database contains **3,000+ stored procedures** that implement the core data operations for the platform. All database reads and writes in the .NET backend go through stored procedures -- there is no inline SQL in the application code.

:::note Auto-Generated Content
This catalog is a placeholder that will be auto-populated by the `parse:dotnet:sprocs` script. Run `npm run parse:dotnet:sprocs` to refresh the stored procedure data from the database.
:::
