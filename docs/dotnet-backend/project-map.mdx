...
import solutionMap from '@site/generated/dotnet-metadata/solution-map.json';

export const graphData = {
  nodes: solutionMap.projects.map(p => ({
    id: p.name,
    group: p.group,
    label: p.name.replace('MyEvaluations.', '')
  })),
  links: solutionMap.dependencies.map(d => ({
    source: d.from,
    target: d.to
  }))
};
...
The `MyEvaluations2009.sln` solution contains **{solutionMap.projectCount} projects** with **{solutionMap.dependencies.length} dependency edges**. This page provides a visual dependency map and categorized listing of all projects.
...
The graph above is auto-generated from `MyEvaluations2009.sln` ({solutionMap.projectCount} projects, {solutionMap.dependencies.length} dependency edges). Drag nodes to rearrange, scroll to zoom, hover for details. Color coding: **Blue** = Business, **Green** = DataAccess/Infrastructure, **Orange** = Web, **Purple** = Services.
:::
