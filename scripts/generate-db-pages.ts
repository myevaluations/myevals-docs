...
${sprocFullData ? `## Stored Procedure Analysis

**${sprocFullData.totalProcedures?.toLocaleString()}** stored procedures analyzed across ${sprocFullData.modules?.length ?? 0} modules.

| Anti-Pattern | Count | Severity |
|-------------|------:|----------|
| Cursors | ${sprocFullData.stats?.antiPatternCounts?.cursors ?? 0} | Critical |
| SELECT * | ${sprocFullData.stats?.antiPatternCounts?.selectStar ?? 0} | High |
| Dynamic SQL | ${sprocFullData.stats?.antiPatternCounts?.dynamicSql ?? 0} | High |
| NOLOCK usage | ${sprocFullData.stats?.antiPatternCounts?.nolockUsage ?? 0} | Medium |
...
async function main(): Promise<void> {
  ...
  // Load SP full data (optional — generated by parse:db:sprocs)
  const sprocFullPath = path.join(DB_SCHEMA_DIR, 'stored-procedures-full.json');
  let sprocFullData: any = null;
  if (await fileExists(sprocFullPath)) {
    sprocFullData = await readJson<any>(sprocFullPath);
    console.log(`  Loading stored-procedures-full.json (${sprocFullData.totalProcedures?.toLocaleString()} procedures)...`);
  }
  ...
  // 1. Overview page
  const overviewMdx = generateOverviewMdx(tablesData, summaryData, indexesData, triggersData, sprocFullData);
